...
public class LavaTankLogic extends MultiServantLogic implements IFluidHandler {
    ...
    public FluidStack drain(ForgeDirection from, FluidStack resource, boolean doDrain) {
        ...
        if (tank.getFluid()
            .getFluid() != resource.getFluid()) return null;
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return tank.getFluidAmount() == 0 || (tank.getFluid()
            .getFluid() == fluid && tank.getFluidAmount() < tank.getCapacity());
    }
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        ...
        if (tank.getFluid() != null) fluid = tank.getFluid()
            .copy();
    ...
    public int getBrightness() {
        if (containsFluid()) {
            return (tank.getFluid()
                .getFluid()
                .getLuminosity() * tank.getFluidAmount()) / tank.getCapacity();
        }
        return 0;
    }
