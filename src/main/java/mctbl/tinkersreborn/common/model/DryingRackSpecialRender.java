...
public class DryingRackSpecialRender extends TileEntitySpecialRenderer {
    ...
    public void render(DryingRackLogic logic, double posX, double posY, double posZ, float var8) {
        ...
        this.renderStackInTile(logic);
        GL11.glPopMatrix();
    }

    private void renderStackInTile(DryingRackLogic logic) {
        ItemStack stack = logic.isStackInSlot(1) ? logic.getStackInSlot(1) : logic.getStackInSlot(0);
    ...
    void renderItem(DryingRackLogic logic, ItemStack stack) {
        ...
        GL11.glPushMatrix();
        int meta = logic.getWorldObj()
            .getBlockMetadata(logic.xCoord, logic.yCoord, logic.zCoord);
        if (meta <= 1) GL11.glTranslatef(1F, -0.375F, 0.905F);
        else {
            ...
            if (meta == 2) GL11.glTranslatef(0F, 0F, 0.375F);
            if (meta == 3) {
                // Rotate, Flip.
