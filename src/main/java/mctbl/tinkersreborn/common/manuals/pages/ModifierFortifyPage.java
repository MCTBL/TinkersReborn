...
public class ModifierFortifyPage extends AbstractManualPage {
    ...
    protected ModifierFortifyPage() {
	super(TinkersRebornUtils.jsonParser.parse(String.format(pattern, "fortify"))
                .getAsJsonObject());
	List<IModifierDisplay> fortifies = TinkersRebornRegistry.getAllModifier().stream().filter(ModFortify.class::isInstance).map(IModifierDisplay.class::cast).collect(Collectors.toList());
        ...
        this.text = "modifier.fortify.desc";
        this.inputStacks = fortifies.stream().map(IModifierDisplay::getItems).flatMap(List::stream).collect(Collectors.toList());
