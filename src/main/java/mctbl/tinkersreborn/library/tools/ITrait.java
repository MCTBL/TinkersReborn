...
public interface ITrait extends IToolMod {
    ...
    /**
     * Called each tick by the tool. See
     * {@link net.minecraft.item.Item#onUpdate(ItemStack, World, Entity, int, boolean)}
     */
    ...
    /**
     * Called after the block has been destroyed. See
     ...
     * @param player     The player (or entity) that is hitting the target.
     * @param target     The entity to hit.
     * @param damage     The original, unmodified damage from the tool. Does not includes critical damage, that will be
     ...
     */
    float damage(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damage, float newDamage,
        boolean isCritical);

    /**
     * Called when an entity is hit, just before the damage is dealt. Damage is the final damage dealt, including
     * critical damage.
     * Damage has been fully calculated. You can deal damage to the entity in this callback.
     * The hurtResistantTime will be set correctly before the call, and it will be reset after the call for the original
     * damage call.
     *
     * @param tool       The tool dealing the damage.
     * @param player     The player (or entity) that is hitting the target.
    ...
    /**
     * Modify the knockback applied. Called after onHit and with the actual damage value. Damage value INCLUDES crit
     * damage here.
     *
     * @param tool         The tool dealing the damage.
     * @param player       The player (or entity) that is hitting the target.
     ...
     */
    float knockBack(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damage, float knockback,
        float newKnockback, boolean isCritical);
     ...
     * @param damageDealt How much damage has been dealt to the entity. This is the ACTUAL damage dealt - the difference
     *                    in Health of the entity.
     * @param wasCritical If the hit was a critical hit.
     * @param wasHit      If the target was actually hit. False when the entity was still invulnerable, or prevented the
     *                    damage because of some other reason.
     */
    void afterHit(ItemStack tool, EntityLivingBase player, EntityLivingBase target, float damageDealt,
    ...
    /**
     * Called when the player holding the tool is damaged. Only called if the player is NOT blocking! onBlock is called
     * in that case.
     */
     ...
     * This callback as well as onToolHeal will be called multiple times when a tool is getting repaired with multiple
     * items.
     *
