...
public abstract class BaseContainer<T extends TileEntity> extends Container {
    ...
    /**
     * Called when the container is opened and another player already has a
     * container for this tile open Sync to the same state here.
     */
    protected void syncWithOtherContainer(BaseContainer<T> otherContainer, EntityPlayerMP player) {}

    /**
     * Called when the container is opened and no other player has it open. Set the
     * default state here.
     */
    ...
    public boolean canInteractWith(@Nonnull EntityPlayer playerIn) {
        ...
        return playerIn.getDistanceSq((double) xCoord + 0.5d, (double) yCoord + 0.5d, (double) zCoord + 0.5d)
            <= maxDist;
    }
    ...
    protected Slot addSlotToContainer(Slot slotIn) {
        ...
        // throw new SlimeknightException("BaseContainer: Player inventory has to be last slots. Add all slots before
    ...
    public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
        ...
        if (playerInventoryStart < 0) {
            // so we don't do anything if no player inventory is present because we don't
            // know what to do
            return null;
        }
    ...
    protected boolean mergeItemStackRefill(@Nonnull ItemStack stack, int startIndex, int endIndex,
        ...
        if (stack.isStackable()) {
            while (stack.stackSize > 0 && (!useEndIndex && k < endIndex || useEndIndex && k >= startIndex)) {
                ...
                if (!(itemstack1 == null || itemstack1.stackSize == 0) && itemstack1.getItem() == stack.getItem()
    ...
    protected boolean mergeItemStackMove(@Nonnull ItemStack stack, int startIndex, int endIndex, boolean useEndIndex) {
        ...
        while (!useEndIndex && k < endIndex || useEndIndex && k >= startIndex) {
            ...
            if ((itemstack1 == null || itemstack1.stackSize == 0) && slot.isItemValid(stack)
                && this.canMergeSlot(stack, slot)) // Forge: Make sure
            // to respect
            // isItemValid in the
            // slot.
            {
