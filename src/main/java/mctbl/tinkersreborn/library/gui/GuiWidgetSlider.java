...
public class GuiWidgetSlider extends GuiWidget {
    ...
    // where the slider was clicked on the slider itself (not on the bar, on the
    // thing that slides)
    private int clickX;
    private int clickY;
    private boolean clickedBar; // if the bar has already been clicked and not released

    public GuiWidgetSlider(GuiElement slider, GuiElement sliderHighlighted, GuiElement sliderDisabled,
        GuiElement slideBarTop, GuiElement slideBarBottom, GuiElementScalable slideBar) {
        this.slider = slider;
        ...
        sliderOffset = (int) (MathHelper.abs(slideBar.w - slider.w)) / 2;
    ...
    public void update(int mouseX, int mouseY, boolean useMouseWheel) {
        ...
        else if (mouseDown && !clickedBar && x >= 0 && y >= 0 && x <= slideBar.w && y <= height) {
            if (y < getSliderTop()) {
                decrement();
            } else {
                increment();
