...
import mctbl.tinkersreborn.library.utils.ItemMetaWrapper;
...
public class Smeltery {
    ...
    /**
     * Adds mappings between an input and its liquid. Renders with the given input's block ID and metadata Example:
     * Smeltery.addMelting(Block.oreIron, 0, 600, new FluidStack(liquidMetalStill.blockID, TConstruct.ingotLiquidValue *
     * 2, 0));
     *
    ...
    /**
     * Adds an alloy mixing recipe. Example: Smeltery.addAlloyMixing(new FluidStack(bronzeID, 2, 0), new
     * FluidStack(copperID, 3, 0), new FluidStack(tinID, 1, 0)); The example mixes 3 copper with 1 tin to make 2 bronze
     *
    ...
    public static void addMelting(FluidType type, ItemStack input, int temperatureDifference, int fluidAmount) {
        ...
        if (input.getItem() instanceof ItemBlock) addMelting(
            ...
            input.getItemDamage(),
            type.baseTemperature + temperatureDifference,
