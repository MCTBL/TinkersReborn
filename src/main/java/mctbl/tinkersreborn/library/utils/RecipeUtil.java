...
public final class RecipeUtil {
    ...
    /**
     * Called by the config to add the preferences. Do not call outside of Tinkers
     * Construct
     * 
     * @param preferences
     */
     ...
     *         This is the same ItemStack from the cache, so be sure to copy it if
     *         you plan to place it in an inventory or otherwise change it.
     */
    ...
    private static ItemStack cachePreference(String oreName) {
        ...
        for (String mod : orePreferences) {
            Optional<ItemStack> optional = items.stream()
                .filter(stack -> GameRegistry.findUniqueIdentifierFor(stack.getItem()).modId.equals(mod))
                .findFirst();
        ...
        if (preference.getItemDamage() == OreDictionary.WILDCARD_VALUE) {
            List<ItemStack> subItems = new ArrayList<>();
            preference.getItem()
                .getSubItems(preference.getItem(), CreativeTabs.tabAllSearch, subItems);
            // just in case
            if (subItems.isEmpty()) {
                // so you have an oredicted item with no sub items? I guess all we can do is
                // give damage 0
