...
public class GuiInfoPanel extends GuiModule {
    ...
    private static ResourceLocation BACKGROUND = new ResourceLocation(TinkersReborn.MODID, "textures/gui/panel.png");
    ...
    private FontRenderer fontRenderer = TinkersRebornGeneralProxyClient.fontRender;
    ...
    public void setText(List<String> text, List<String> tooltips) {
        ...
        if (text != null) {
            ...
            for (int i = 0; i < text.size(); i++) {
                text.set(i, TinkersRebornUtils.convertNewlines(text.get(i)));
            }
        }
        this.text = text;
        updateSliderParameters();
    ...
    protected void setTooltips(List<String> tooltips) {
        ...
        if (tooltips != null) {
            for (int i = 0; i < tooltips.size(); i++) {
                tooltips.set(i, TinkersRebornUtils.convertNewlines(tooltips.get(i)));
            }
        }
    ...
    private void shiftSlider(int xd, int yd) {
        slider = new GuiWidgetSlider(
            sliderNormal.shift(xd, yd),
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        if (hasTooltips() && mouseX >= guiRight() - border.w - fontRenderer.getCharWidth('?') / 2
            && mouseX < guiRight()
            && mouseY > guiTop + 5
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        if (hasTooltips()) {
            fontRenderer.drawString(
                "?",
                guiRight() - border.w - fontRenderer.getCharWidth('?') / 2,
                guiTop + 5,
                0xff5f5f5f,
                false);
        }
        ...
        while (iter.hasNext()) {
            ...
            fontRenderer.drawStringWithShadow(line, (int) x, (int) y, color);
            y += textHeight;
        }

        GL11.glScalef(1f / textScale, 1f / textScale, 1.0f);
